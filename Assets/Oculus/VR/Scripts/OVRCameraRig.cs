...
public class OVRCameraRig : MonoBehaviour
{
	...
	private Vector3 _trainingEndPositionLeft;
	private Vector3 _trainingEndPositionRight;
	private float _pseudoRange;
	private Vector3 _prepreRightHandPosition = Vector3.zero;
	private Vector3 _preRightHandPosition = Vector3.zero;
	private Vector3 _preLeftHandPosition = Vector3.zero;
	private Vector3 _prepreLeftHandPosition = Vector3.zero;
	private bool _earlyLift = false;
	private Vector3 rightPosSetting = Vector3.zero;
	private Vector3 leftPosSetting = Vector3.zero;
	private float _afterEarlyLift;
	private int _trainingType;
	private float _pseudoTime;
	private float _timerLeft = 0.0f;
	private bool _liftStart = false;
	private bool _overTheStartLine = false;
	private float _aheadMergin;
...
#region Unity Messages
	...
	public Vector3 TrainingStart()
	{
		...
		else if(_trainingType == 1)
		{
			return leftHandAnchor.position;
		}
		else{
			return (_trainingBasePositionLeft + _trainingBasePositionRight) / 2;
		}
	}
	...
	private void setHandAnchor()
	{
		...
		//加速度を算出
		Vector3 preLeftVelocity = _preLeftHandPosition - _prepreLeftHandPosition;
		Vector3 LeftVelocity = leftPos - _preLeftHandPosition;
		Vector3 preRightVelocity = _preRightHandPosition - _prepreRightHandPosition;
		Vector3 RightVelocity = rightPos - _preRightHandPosition;
		
		float leftAcceleration = LeftVelocity.magnitude - preLeftVelocity.magnitude;
		float rightAcceleration = RightVelocity.magnitude - preRightVelocity.magnitude;

		_prepreLeftHandPosition = _preLeftHandPosition;
		_preLeftHandPosition = leftPos;
		_prepreRightHandPosition = _preRightHandPosition;
		_preRightHandPosition = rightPos;
		bool preOverTheStartLine = _overTheStartLine;
		bool upward = false;
		...
		if(_trainingType == 0)
		{
			_earlyLift = (rightAcceleration > 0) && (RightVelocity.y > 0);
		}
		else if(_trainingType == 1)
		{
			_earlyLift = (leftAcceleration > 0) && (LeftVelocity.y > 0);
		}
		else
		{
			_earlyLift = (leftAcceleration > 0) && (rightAcceleration > 0) && (RightVelocity.y > 0) && (LeftVelocity.y > 0);
		}
		*/

		if(_trainingType == 0)
		{
			_overTheStartLine = rightPos.y > _trainingBasePositionRight.y;
			upward = RightVelocity.y > 0;
		}
		else if(_trainingType == 1)
		{
			_overTheStartLine = leftPos.y > _trainingBasePositionLeft.y;
			upward = LeftVelocity.y > 0;
			
		}
		else
		{
			_overTheStartLine = (rightPos.y > _trainingBasePositionRight.y) && (leftPos.y > _trainingBasePositionLeft.y);
			upward = RightVelocity.y > 0;
			upward  = upward && (LeftVelocity.y > 0);
		}
		...
		if(_isTraining)
		{
			...
			{
				Vector3 leftGap = _trainingEndPositionLeft - leftPosSetting;
				Vector3 rightGap = _trainingEndPositionRight - rightPosSetting;
				float scale = _afterEarlyLift/rightGap.magnitude;
				rightPosSetting = rightPosSetting + RightVelocity + Vector3.Scale(rightGap,new Vector3(scale,scale,scale));
				scale = _afterEarlyLift/leftGap.magnitude;
				leftPosSetting = leftPosSetting + LeftVelocity + Vector3.Scale(leftGap,new Vector3(scale,scale,scale));
			}
			else
			{
				
				leftPosSetting = leftPos;
				rightPosSetting = rightPos;
			}
			...
			{
				Vector3 leftGap = leftPosSetting - leftPos;
				Vector3 rightGap = rightPosSetting - rightPos;
				rightPosSetting = rightPos + Vector3.Scale(rightGap,_afterEarlyLift);
				leftPosSetting = leftPos + Vector3.Scale(leftGap,_afterEarlyLift);
			}
			*/
		}
		else
		...
		leftPosSetting.z += _aheadMergin;
		rightPosSetting.z += _aheadMergin;
		leftHandAnchor.localPosition = leftPosSetting;
		rightHandAnchor.localPosition = rightPosSetting;
		leftHandAnchor.localRotation = OVRInput.GetLocalControllerRotation(OVRInput.Controller.LTouch);
		rightHandAnchor.localRotation = OVRInput.GetLocalControllerRotation(OVRInput.Controller.RTouch);
	}
	...
	public Vector3 RegisterEnd()
	{
		_trainingEndPositionLeft = leftHandAnchor.localPosition;
		_trainingEndPositionRight = rightHandAnchor.localPosition;
		if(_trainingType == 0)
		{
			return rightHandAnchor.position;
		}
		...
		{
			return _trainingBasePositionLeft;
		}
		else{
			return (_trainingBasePositionLeft + _trainingBasePositionRight) / 2;
		}
	...
	public void setParams(float pseudoRange,float afterEarlyLiftScale,int trainingType,float pseudoTime,float aheadMergin)
	{
		_pseudoRange = pseudoRange;
		_trainingType = trainingType;
		_pseudoRange = pseudoRange;
		_afterEarlyLift = afterEarlyLiftScale;
		_aheadMergin = aheadMergin;
	}
}
