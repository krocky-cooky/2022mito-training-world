...
namespace Fishing.State
{
    ...
    public class DuringFishing_Calibration : MasterStateBase
    {
        ...
        // キャリブレーション時の速度上限
        private float _spdLimitLiftup = 2.0f;

        public List<float> _processedMeasuredTorques;
        ...
        public override void OnExit()
        {
            ...
            {
                // 計測データを処理したもの
                _processedMeasuredTorques = new List<float>(master.measuredTorques);
        ...
        public override int StateUpdate()
        {
            ...
            // モータへの指令
            // 速度上限つきのトルク指令
            master.device.SetTorqueMode(master.minUserPower, 6.0f, _spdLimitLiftup);


            // トルクとポジションの記録
            // 速度制限にかかっているときだけ記録
            if (master.device.speed <= (- (_spdLimitLiftup - 0.05f))){
            ...
            if (master.trainingDevice.currentNormalizedPosition >= 0.99f){
